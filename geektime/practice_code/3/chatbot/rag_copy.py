...
def indexing(
  ...
  try:
    ...
    logging.error(f"creat index failuer: {str(e)}")
    raise
...
def create_index(document_path: str = "./docs") -> VectorStoreIndex:
  ...
  if not Path(document_path).exists():
    raise FileNotFoundError(f"file is not exist: {document_path}")

  documents = SimpleDirectoryReader(document_path).load_data()
  # 创建一个自定义的节点解析器
  # from llama_index.core import VectorStoreIndex
  # from llama_index.core.node_parser import SentenceSplitter
  # node_parser = SentenceSplitter(chunk_size=256, chunk_overlab=20)
  ...
  index = VectorStoreIndex.from_documents(
    ...
    # ),
    # node_parser = node_parse # 使用自定义的节点解析器
  )
  return index
...
def create_query_engine(index):
  ...
  if not api_key:
    raise ValueError("please config DASHSCOPE_API_KEY environment variable")
...
# query_engine = index.as_query_engine(
#   node_postprocessors = [
#     CustomPostprocessor(custome_rank)
#   ]
# )
...
def ask(
  ...
  if not question.strip():
    raise ValueError("question should not be blank.")
...
if __name__ == "__main__":
  ...
  # 创建查询引擎
  ask("please give the introduction of RAG in 100 words", query_engine, stream=True, debug=True)
